...
always@(OP_CODE or FUNC) begin
    ...
    ALU_OP = 
        ...
        XORI?9:
        0;
    MemToReg = (LW )? 1 : 0;//removed lh from it
    MemWrite = SW ? 1 : 0;
    ...
    SysCALL = SYSCALL ? 1 : 0;
    SignedExt = (BEQ | BNE | ADDI | ADDIU | SLTI | LW | SW | LH |BGTZ) ? 1 : 0;
    ...
    LH = LH ?1:0;
    BGTZ = BGTZ?1:0;
    R1_USED = (ADD |ADDU |SUB |AND |OR |NOR |SLT |SLTU |JR |SYSCALL |BEQ|BNE |ADDI |ANDI |ADDIU |SLTI|ORI|LW|SW|SV|XORI|LH|BGTZ)?1:0;
    R2_USED = (SLL| SRA| SRL| ADD |ADDU |SUB |AND |OR |NOR |SLT |SLTU |SYSCALL |BEQ|BNE |SW|SV)?1:0;
end
